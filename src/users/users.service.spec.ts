...
describe('UsersService', () => {
  ...
  describe('create', () => {
    it('should hash the password and create a user', async () => {
      ...
      expect(prismaService.user.create).toHaveBeenCalledWith({ data: { ...createUserDto, password: hashedPassword } });
    });
  });

  describe('findAll', () => {
    it('should return an array of users', async () => {
      const users = [{ id: 1, email: 'test@example.com', password: 'hashedPassword', deletedAt: null }];
  ...
  describe('findOne', () => {
    it('should return a user by ID', async () => {
      const user = { id: 1, email: 'test@example.com', password: 'hashedPassword', deletedAt: null };
      mockPrismaService.user.findUnique.mockResolvedValue(user);

      expect(await service.findOne(1)).toEqual(user);
      expect(prismaService.user.findUnique).toHaveBeenCalledWith({ where: { id: 1, deletedAt: null } });
    });
  });
  ...
  describe('update', () => {
    it('should hash the new password if provided and update the user', async () => {
      ...
      const updatedUser = { id: 1, email: 'test@example.com', password: hashedPassword };
      ...
      expect(bcrypt.hash).toHaveBeenCalledWith('newPassword123', roundsOfHashing);
      expect(prismaService.user.update).toHaveBeenCalledWith({ where: { id: 1 }, data: { ...updateUserDto, password: hashedPassword } });
    });
  })
})